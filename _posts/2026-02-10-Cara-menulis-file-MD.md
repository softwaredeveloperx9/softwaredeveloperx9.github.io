---
layout: post
title: "Cara menulis file .MD"
date: 2026-02-10 09:30:00 +0700
categories: [kategori1, kategori2]
tags: [tag1, tag2, tag3]
author: SDR - Software DevelopeR
---

# Cara menulis file .MD

> **keterangan:** panduan untuk menulis file .MD yang efektif dan efisien.

## Daftar isi
* TOC
{:toc}

## Tulisan Umum

1. selalu gunakan `GIT History`<br/>
untuk _track changes_

2. perhatikan cara menulis<br/>
Tulisan akan dibaca oleh manusia, maka sebaiknya menulis secara `natural`.
    - Indentasi
    - Outline

> **Note:** AI sangat cerdas dan langsung paham dengan mudah, tulisan yang dibuat secara `natural`.

3. SDR:
    - **S**ituation
    - **D**efine {Define, Design, Document, Develop, Deliver}
    - **R**esult {Realistic}
   
4. Too Few vs Too Much

```
    ❌ Too Few   → Incomplete, tidak berguna
    ❌ Too Much  → Overwhelmed, kebanjiran informasi
    ✅ Pragmatic
```
  
5. jika memungkinkan, berisi 3 hal berikut:<br/>
{_Nice to have_, tetapi tidak harus selalu demikian, _it can be boring_}
    1. Problem
    2. Solusi
    3. Ide Utama
  
```
## Problem and Solution

**Problem:**
- user ingin melakukan Import data ke Accurate
- Accurate mempunyai Format data khusus untuk masing-masing data

**Solution:** 
- tulisan `step by step` untuk melakukan Import data ke Accurate

## Ide Utama: buat Data sesuai Format, file contoh dari Accurate
- file Excel dari User
- `Contoh File` {_file Template Excel_} dari Accurate
- Generate data (kombinasi)
```

## Tulisan Teknis

### Filosofi:

1. the Pragmatic Programmer [book](https://en.wikipedia.org/wiki/The_Pragmatic_Programmer)
2. Solve the problem
3. not Perfectionist
   - hindari `Over Engineering`
   - Good > Perfect
   - Shipment {Delivery} the Result {tulisan, `Code`, ...}
4. CI / CD
   - Continuous Improvement / Continuous Development
   - bertahap
   - terus menerus {*Continuous*}
5. SDR:
    - **S**ituation
    - **D**efine {Develop}
    - **R**esult {Realistic}
